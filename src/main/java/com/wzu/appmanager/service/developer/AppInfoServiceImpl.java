...
import com.wzu.appmanager.dao.AppInfoMapper;
import com.wzu.appmanager.dao.AppVersionMapper;
...
public class AppInfoServiceImpl implements AppInfoService {
	...
	@Override
	public boolean add(String softwarename, String apkname, String supportrom, String interfacelanguage, Date updatedate, BigDecimal softwaresize, Long devid, String appinfo, Integer status, Date onsaledate, Date offsaledate, Long categorylevel1, Long categorylevel2, Long categorylevel3, Long downloads, Long flatformid, String logopicpath, String logolocpath, Long createdby, Date creationdate) throws Exception {
		boolean flag = false;
		if(mapper.add(softwarename,apkname,supportrom,interfacelanguage,updatedate,softwaresize,devid,appinfo,status,onsaledate,offsaledate,categorylevel1,categorylevel2,categorylevel3,downloads,flatformid,logopicpath,logolocpath,createdby,creationdate) > 0){
			flag = true;
		}
		return flag;
	}
	...
	@Override
	public boolean modify(Long id, String softwarename, String apkname, String supportrom, String interfacelanguage, Date updatedate, BigDecimal softwaresize, Long devid, String appinfo, Integer status, Date onsaledate, Date offsaledate, Long categorylevel1, Long categorylevel2, Long categorylevel3, Long downloads, Long flatformid, String logopicpath, String logolocpath, Long createdby, Long modifyBy, Date creationdate,Date modifyDate) throws Exception {
		// TODO Auto-generated method stub
		boolean flag = false;
		if(mapper.modify(id,softwarename,apkname,supportrom,interfacelanguage,updatedate,softwaresize,devid,appinfo,status,onsaledate,offsaledate,categorylevel1,categorylevel2,categorylevel3,downloads,flatformid,logopicpath,logolocpath,createdby,modifyBy,creationdate,modifyDate) > 0){
			flag = true;
		}
		return flag;
	}
	...
	public boolean appsysUpdateSaleStatusByAppId(AppInfo appInfoObj) throws Exception {
		...
		Integer operator = Math.toIntExact(appInfoObj.getModifyby());
		if(operator < 0 || appInfoObj.getId() < 0 ){
			throw new Exception();
		}
		
		//get appinfo by appid
		AppInfo appInfo = (AppInfo) mapper.getAppInfo(Math.toIntExact(appInfoObj.getId()), null);
		if(null == appInfo){
			return false;
		}else{
	...
	private boolean offSale(AppInfo appInfo,Integer operator,Integer appInfStatus) throws Exception{
		...
		_appInfo.setModifyby(Long.valueOf(operator));
		_appInfo.setOffsaledate(new Date(System.currentTimeMillis()));
		mapper.modify(appInfo.getId(),appInfo.getSoftwarename(), appInfo.getApkname(), appInfo.getSupportrom(), appInfo.getInterfacelanguage(), appInfo.getUpdatedate(),
				...
				appInfo.getCategorylevel2(), appInfo.getCategorylevel3(), appInfo.getDownloads(), appInfo.getFlatformid(), appInfo.getLogopicpath(), appInfo.getLogolocpath(), appInfo.getCreatedby(),appInfo.getModifyby(),
				appInfo.getCreationdate(),appInfo.getModifydate());
		return true;
	}
	...
	private boolean setSaleSwitchToAppVersion(AppInfo appInfo,Integer operator,Integer saleStatus) throws Exception{
		...
		appVersion.setId(Math.toIntExact(appInfo.getVersionid()));
